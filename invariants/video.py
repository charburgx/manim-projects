...
class Friend(VGroup):
    ...
    def __init__(self, **kwargs):
        ...
        self.elements = VGroup(self.arm1, self.arm2, self.body, self.mango1, self.mango2)
    ...
    EAT_POS = ((0, 0, 0),  ((0.6, -0.25, 0), SHARE_POS[1][1]), ( (0, 0, 0), SHARE_POS[2][1] ) )
...
class FriendCircle(VGroup):
    ...
    def __init__(self, n, init_state, **kwargs):
        ...
        for i in range(0, n):
            ...
            counter = Integer(init_state[i], edge_to_fix=ORIGIN)
            counter.scale(self.text_size)
            counter.move_to(ap[1])
            counter.shift(0.04 * (self.text_size/0.7) * DOWN)
...
class Testing2(Scene):
    def construct(self):
        friend_circle = FriendCircle(8, [2019, 0, 0, 0, 0, 0, 0, 0], p_eat=1)
